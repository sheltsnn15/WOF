...
class Guess_That_Phrase:
    ...
    def main_display(self, phrase_to_guess):
        ...
        print(f'  {phrase:^{54}} ')
        print("                                                          ")
        print(f'     {"".join(self.VOWELS)}        {"".join(self.CONSONANTS)}          ')
        print(f'                                                         ')
    ...
    def process_user_action(self, choice, phrase_to_guess, loaded_phrase, sentence, vowels_picked,
                            consonants_picked):
        solved = False
        end_game = False
        if choice == 1:
            self.guess_consonant(phrase_to_guess, loaded_phrase, consonants_picked)
        elif choice == 2:
            self.guess_vowel(phrase_to_guess, loaded_phrase, vowels_picked, consonants_picked)
        elif choice == 3:
            solved = self.guess_phrase(phrase_to_guess, sentence)
        elif choice == 4:
            end_game = True
            solved = True
    ...
    def guess_consonant(self, phrase_to_guess, loaded_phrase, consonants_picked):
        carry_on = True
        if len(set(self.CONSONANTS)) == 1:
            print("There are no more consonants to choose.")
            carry_on = False
        else:
            consonant = (str(input("Pick a consonant: "))).lower()
        num_consonants = 0
        while (consonant.upper() not in self.CONSONANTS) and carry_on == True:
            if consonant.upper() in self.VOWELS:
        ...
        self.CONSONANTS[self.CONSONANTS.index(consonant.upper())] = ' '
        if num_consonants == 1:
    ...
    def guess_vowel(self, phrase_to_guess, loaded_phrase, vowels_picked, consonants_picked):
        num_vowel = 0
        carry_on = True
        vowel = (str(input("Pick a vowel: "))).lower()
        while (vowel not in loaded_phrase) and (carry_on == True):
            if vowel.upper() in self.VOWELS:
                print(f"I'm sorry, there are no {vowel.upper()}'s.\n")
                self.VOWELS[self.VOWELS.index(vowel.upper())] = ' '
                break
            elif (vowel.upper() in self.CONSONANTS) or (vowel.upper() in consonants_picked):
                print("Consonants cannot be picked.")
            elif len(vowel) != 1:
                print(f"{GameErrorCodes.ONLY_ONE_CHARACTER}")
            elif len(set(self.VOWELS)) == 1:
        ...
        while vowel in loaded_phrase:
            if (vowel.upper() in self.CONSONANTS) or (vowel.upper() in consonants_picked):
                print("Consonants cannot be picked.")
                vowel = (str(input("Pick a vowel: "))).lower()
            else:
        ...
        if vowel.upper() in self.VOWELS:
            index = self.VOWELS.index(vowel.upper())
            self.VOWELS[index] = ' '
        ...
        while new_game != "N":
            ...
            while not end_game:
                ...
                while not solved:
                    ...
                    solved, end_game = self.process_user_action(choice, phrase_to_guess, loaded_phrase,
